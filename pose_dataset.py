...
# 用于返回动作名
ACTION_NAMES = [''] * 15
ACTION_NAMES[0] = 'Directions'
ACTION_NAMES[1] = 'Discussion'
ACTION_NAMES[2] = 'Eating'
ACTION_NAMES[3] = 'Greeting'
ACTION_NAMES[4] = 'Phoning'
ACTION_NAMES[5] = 'Photo'
ACTION_NAMES[6] = 'Posing'
ACTION_NAMES[7] = 'Purchases'
ACTION_NAMES[8] = 'Sitting'
ACTION_NAMES[9] = 'SittingDown'
ACTION_NAMES[10] = 'Smoking'
ACTION_NAMES[11] = 'Waiting'
ACTION_NAMES[12] = 'WalkDog'
ACTION_NAMES[13] = 'Walking'
ACTION_NAMES[14] = 'WalkTogether'
##################################################################
...
def project_point_radial(P, R, T, f, c, k, p):
    ...
    # R矩阵乘(P的转置-T)
    #########################################################################
    X = R.dot(P.T - T)  # rotate and translate // dot矩阵乘法 .T转置
    #########################################################################
    # XX = 用xy坐标的矩阵/z坐标的矩阵
    XX = X[:2, :] / X[2, :] # X[:2, :] // 列全取 行取两行
    # r2 = x/z得到的矩阵的平方 + y/z得到的矩阵的平方
    r2 = XX[0, :] ** 2 + XX[1, :] ** 2 # ** 2表示矩阵乘方

    # np.tile k行向复制N倍，纵向复制1倍
    radial = 1 + np.einsum(
        'ij,ij->j', np.tile(k, (1, N)), np.array([r2, r2 ** 2, r2 ** 3]))
    #XX乘以相机径向畸变系数
    tan = p[0] * XX[1, :] + p[1] * XX[0, :]

    # np.outer para1的第一个元素与para2的每一个元素分别相乘得到第一行
    #                 第二个元素与para2的每一个元素分别相乘得到第二行
...
class H36M(pose_dataset_base.PoseDatasetBase):
    ...
    def __init__(self, action='all', length=1,
        ...
        # 要使用的关节位置索引(17点)
        # 坐标的排布是按照 x1,y1,z1 ... xn, yn, zn 进行排布的
    ...
    def get_example(self, i):
        ...
        # 上面有 if use_sh的这一步操作，不用sh_detection也要加上这一步操作
        ######################################################################
        if 'TakingPhoto' in file_name:
            file_name = file_name.replace('TakingPhoto', 'Photo')
        if 'WalkingDog' in file_name:
        ...
        X, scale = self._normalize_3d(X) # X先变换 再normalize 两个视角的同一个3D scale也不同
        ...
        # a确定为动作下标
        a = 0
        while ACTION_NAMES[a] != file_name.split()[0]:
            a = a + 1
...
class MPII(chainer.dataset.DatasetMixin):
    ...
    def get_example(self, i):
        ...
        mpii_1 = np.zeros((1 ,34), dtype=np.float32)
        mpii_1[0, 0::1] = mpii_poses[0, 0, 0::1]

        return mpii_1, dummy_X, dummy_scale
a = []
a.